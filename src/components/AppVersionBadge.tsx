...
import { currentRoutePath, onRouteChange } from '../lib/route'
...
//
// Read from the route rather than passed down, because this is mounted at the
...
function onAllowedRoute() {
  try {
    return SHOWN_ON.includes(currentRoutePath())
  } catch {
    return false
  }
}
...
export default function AppVersionBadge() {
  ...
  useEffect(() => {
    if (!isDesktop) return
    return onRouteChange(() => setVisible(onAllowedRoute()))
  }, [])
