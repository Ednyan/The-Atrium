...
// Who this Windows account is, as far as a vault is concerned.
//
// Stored in app_data_dir, which is per-Windows-user, so two accounts on one
// machine get different ids while each keeps the same one forever. That is the
// whole point: a vault shared between family members needs to tell them apart,
// and the only thing that reliably distinguishes them is which account is
// logged in.
//
// The id is generated by the caller rather than here, because there is no uuid
// crate in this build and adding one to write a single value would be a poor
// trade. `proposed` is only used when there is nothing on disk yet -- the file
// wins every time after that, so calling this repeatedly cannot change who
// somebody is.
//
// os_user is a suggestion for a display name and nothing more. It is never an
// identity: Windows accounts can be renamed, and two machines can both have an
// "Admin".
...
fn device_identity_path(app: &tauri::AppHandle) -> Result<PathBuf, String> {
    let app_data = app.path().app_data_dir().map_err(|e| e.to_string())?;
    Ok(app_data.join("identity.json"))
}

#[tauri::command]
fn get_or_create_device_id(app: tauri::AppHandle, proposed: String) -> Result<DeviceIdentity, String> {
    ...
    if path.exists() {
        ...
        if let Ok(mut existing) = serde_json::from_str::<DeviceIdentity>(&contents) {
            if !existing.device_id.trim().is_empty() {
                // Refreshed on every read: a renamed Windows account should
                // suggest its new name to any vault it has not met yet, without
                // disturbing the id or any name already chosen.
                existing.os_user = os_user;
                return Ok(existing);
            }
        }
        // Unreadable or empty. Falls through and writes a new one rather than
        // refusing to start -- an identity file that cannot be parsed is not
        // worth stranding somebody over.
    }

    let trimmed = proposed.trim();
    if trimmed.is_empty() {
        return Err("No identity was proposed".into());
    }
